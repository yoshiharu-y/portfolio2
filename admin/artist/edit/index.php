...
//////////////////////////////////////////////////
//
//　　　artist/edit内のindex.php
//
//   本サーバアップ時に各editファイル内にある
//   $artistdirが本サーバ仕様になってるか確認する
//////////////////////////////////////////////////
...
if($_SESSION["mms_admin_login"])
{
	if($_SESSION["edit_start"])
	{
		...
		{
			//画面指定
			define("FILENAME","artist_edit_check");
	
		}
		else if(!empty($_POST["artist_edit_return"]))
		{
			//画面指定
			define("FILENAME","artist_edit");
		}
		else if(!empty($_POST["artist_edit_comp"]))
		{
			...
			{
				//画面指定
				define("FILENAME","artist_edit_complate");
			}
			else
			{	
				//画面指定
		...
		//編集中に戻るボタンを押しPOST値に$_POST["artist_edit"]が入ってしまった場合
		else
		{
			//画面指定
			define("FILENAME","artist_edit");
	...
	else if(!empty($_POST["artist_edit"]))
	{
		//初回時の設定
		$_SESSION["edit_start"]=true;
		$_SESSION["artist_edit"]=array();
		$_SESSION["file_temp"]=array();
		$_SESSION["file_temp"]["seq_num"]=$_POST["artist_no"];
		//画面指定
		define("FILENAME","artist_edit");	
	}
	else
