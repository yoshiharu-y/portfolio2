...
//データをDBから引っ張り出す。
$sql = "SELECT * FROM user where del_flag = 0 order by seq_num desc";
$result_id = $db->ExecuteSQL($sql);

//[for_user]～[end_for_user]で置換する回数
$row_num=0;
...
while($sql_data = $db->FetchRow($result_id))
{
	...
	//[for_user]～[end_for_user]で置換する分だけrowに多次元配列として作成
	$row["user_data_".$row_num]=$sql_data;
	$row_num++;
}
...
/*
html内の[for_user]～[end_for_user]を抽出する
*/

//htmlの[for_rank]より前と後を分割して配列に入れる。
$for = explode("[for_user]",$control->GetContentData());

//htmlの[for_rank]より前を削除
array_shift($for);

//htmlの[end_for_rank]より前と後を分割する。
$for = explode("[end_for_user]",$for[0]);
...
for($i=0; $i < $row_num ;$i++)
{
	...
	foreach( $row["user_data_".$i] as $key => $value )
	{
...
/*
初期化されたHTMLデータ内の[for_user]～[end_for_user]で囲まれたところを
[change_for:dl_for_user]に置換
*/

$htmlFor="[for_user]".$for[0]."[end_for_user]";
$forName="table_for_user";
