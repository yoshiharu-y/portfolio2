...
{
	//確認画面
	if(!empty($_POST["account_edit_check"]))
	{
		//画面指定
		define("FILENAME","account_edit_check");

	}
	//編集画面
	else if(!empty($_POST["account_edit_return"]))
	{
		//画面指定
		define("FILENAME","account_edit");
	}
	//編集完了
	else if(!empty($_POST["account_edit_comp"]))
	{
		//完了チェックがはいっているかどうか
		if(!empty($_POST["comp_check"]))
		{
			//画面指定
			define("FILENAME","account_edit_complate");
		}
		else
		{	
			//画面指定
